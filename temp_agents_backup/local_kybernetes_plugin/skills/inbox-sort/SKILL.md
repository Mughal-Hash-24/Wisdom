...
> - **PLATFORM SECURITY CONSTRAINT:** The Antigravity platform blocks direct execution of custom subagents (e.g. TypeName `classifier`, `librarian`, etc. will error with "not allowed to be invoked").
> - Therefore, you MUST emulate these subagents by spawning `invoke_subagent` with `TypeName: "self"` and copying the target agent's system instructions from `/home/ibtasaam/Kybernetes/.agents/plugins/kybernetes-plugin/agents/{agent_name}/agent.json` (under `customAgent.systemPromptSections`) into the `Prompt` field alongside the specific task.
> - You do NOT classify domains directly. Emulate `@classifier` via `self` for that.
> - You do NOT prepare files directly. Call the `prepare_dispatch` tool.
> - You do NOT generate expansions directly. Emulate the domain agent (`@{domain}`) via `self`.
> - You do NOT stitch files. Call the `stitch_files` tool.
> - You do NOT organize files directly. Emulate `@librarian` via `self`.
...
**B1: Classify System**
- Call the `@classifier` agent (emulated via `self` subagent loading `classifier/agent.json` instructions) with the block's `prompt` to determine the `domain` (e.g., `@turing`).
...
- Call the assigned domain agent (`@{domain}`) (emulated via `self` subagent loading `agents/{domain}/agent.json` instructions) and provide it with:
  1. The user's `prompt`.
...
- Call `@classifier` (emulated via `self` subagent loading `classifier/agent.json` instructions) with the exact `prompt` for ALL blocks in this section **simultaneously**.
...
- Call `@{domain}` (emulated via `self` subagent loading `agents/{domain}/agent.json` instructions) **simultaneously** for all blocks using the prompt, card_content, and **temp_file** (passed as `target_file`) from their payloads.
...
2. **Classify:** Call `@librarian` (emulated via `self` subagent loading `librarian/agent.json` instructions) with the exact file path. Wait for it to read the note and return the strict JSON routing payload.
3. **Hierarchy & Routing:** 
